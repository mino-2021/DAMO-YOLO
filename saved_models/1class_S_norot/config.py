...
class Config(MyConfig):
    def __init__(self):
        ...
        # Total epochs
        self.train.total_epochs = 50

        # Pretrained model path for finetuning
        self.train.finetune_path = './pretrained_models/damoyolo_tinynasL20_T.pth'

        # optimizer - reduced batch size for small dataset
        self.train.batch_size = 8  # Small dataset, use smaller batch
        ...
        # Dataset S (no rotation augmentation)
        self.dataset.train_ann = ('S_norot_train_coco', )
        self.dataset.val_ann = ('S_norot_val_coco', )
        ...
        # Single class detection (S class only)
        ZeroHead = {
            'name': 'ZeroHead',
            'num_classes': 1,  # Only 1 class (S)
